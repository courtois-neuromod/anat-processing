...
label_if_does_not_exist ${file_t2} ${file_t2_seg} "2,5" t2
...
sct_qc -i ${file_t2}.nii.gz -s label_T2w/template/PAM50_levels.nii.gz -p sct_label_vertebrae -qc ${PATH_QC} -qc-subject ${SUBJECT}
# Compute average cord CSA between C2 and C3
sct_process_segmentation -i ${file_t2_seg}.nii.gz -vert 2:3 -vertfile ${file_t2_seg}_labeled.nii.gz -o ${PATH_RESULTS}/csa-SC_T2w.csv -append 1
...
sct_register_multimodal -i label_T2w/template/PAM50_levels.nii.gz -d ${file_t1_seg}.nii.gz -o PAM50_levels2${file_t1}.nii.gz -identity 1 -x nn
# Compute average cord CSA between C2 and C3
sct_process_segmentation -i ${file_t1_seg}.nii.gz -vert 2:3 -vertfile PAM50_levels2${file_t1}.nii.gz -o ${PATH_RESULTS}/csa-SC_T1w.csv -append 1
...
file_t1ax_seg=$FILESEG
# Create mask
sct_create_mask -i ${file_t1ax}.nii.gz -p centerline,${file_t1ax_seg}.nii.gz -size 35mm -o ${file_t1ax}_mask.nii.gz
# Crop data for faster processing
sct_crop_image -i ${file_t1ax}.nii.gz -m ${file_t1ax}_mask.nii.gz -o ${file_t1ax}_crop.nii.gz
file_t1ax="${file_t1ax}_crop"
# Register PD->T1w
# Tips: here we only use rigid transformation because both images have very similar sequence parameters. We don't want to use SyN/BSplineSyN to avoid introducing spurious deformations.
sct_register_multimodal -i ${file_mtoff}.nii.gz -d ${file_t1ax}.nii.gz -dseg ${file_t1ax_seg}.nii.gz -param step=1,type=im,algo=rigid,slicewise=1,metric=CC -x spline -qc ${PATH_QC} -qc-subject ${SUBJECT}
...
mv warp_PAM50_t12${file_t1ax}.nii.gz warp_template2T1ax.nii.gz
mv warp_${file_t1ax}2PAM50_t1.nii.gz warp_T1ax2template.nii.gz
# Warp template
sct_warp_template -d ${file_t1ax}.nii.gz -w warp_template2T1ax.nii.gz -ofolder label_MTS -qc ${PATH_QC} -qc-subject ${SUBJECT}
# Compute MTR
sct_compute_mtr -mt0 ${file_mtoff}.nii.gz -mt1 ${file_mton}.nii.gz
# Compute MTsat
sct_compute_mtsat -mt ${file_mton}.nii.gz -pd ${file_mtoff}.nii.gz -t1 ${file_t1ax}.nii.gz
# Extract MTR, MTsat and T1 in WM between C2 and C5 vertebral levels
sct_extract_metric -i mtr.nii.gz -f label_MTS/atlas -l 51 -vert 2:5 -vertfile label_MTS/template/PAM50_levels.nii.gz -o ${PATH_RESULTS}/MTR.csv -append 1
